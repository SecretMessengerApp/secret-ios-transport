...
class BackendTrustProviderTests: XCTestCase {
    ...
    func trustWithChain(certificateData: [Data], file: StaticString = #file, line: UInt = #line) -> SecTrust? {
    ...
    func testPinnedHostsAreNotTrustedWithWrongCertificate() {
        // given
        guard let serverTrust = trustWithChain(certificateData: certificates.external) else { XCTFail("Failed to create trust"); return }
    ...
    func testExternalHostWithInvalidCertificateIsNotTrusted() {
        // given
        guard let serverTrust = trustWithChain(certificateData: certificates.invalid) else { XCTFail("Failed to create trust"); return }
